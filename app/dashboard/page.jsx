...
export default function DashboardPage() {
  ...
  const fetchRides = async () => {
    try {
      ...
      toast({
        title: "Error",
        description: "Failed to fetch ride history",
        variant: "destructive",
      })
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">Book your ride or track your current journey</p>
        </div>
                  ...
                  <div className="flex items-center justify-between">
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">From: {currentRide.pickup}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <MapPin className="h-4 w-4 text-red-600" />
                        <span className="text-sm">To: {currentRide.dropoff}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        {React.createElement(rideTypeIcons[currentRide.rideType], { className: "h-4 w-4" })}
                        <span className="text-sm capitalize">{currentRide.rideType}</span>
                      </div>
                    </div>
            ...
            <Card>
              ...
              <CardContent>
                <form onSubmit={handleBookingSubmit} className="space-y-4">
                  ...
                  {bookingForm.rideType && (
                    <div className="space-y-2">
                      ...
                      {ridersLoading ? (
                        ...
                        <p className="text-sm text-gray-500 py-2">No riders available for {bookingForm.rideType}</p>
                      ) : (
                              ...
                              <div className="flex items-center justify-between">
                                <div>
                                  ...
                                  <p className="text-sm text-gray-600">
                                    {rider.vehicleType} • ⭐ {rider.rating || "New"} • {rider.totalRides} rides
                                  </p>
                                </div>
                                <div className="flex items-center gap-2">
                  ...
                  <Button type="submit" className="w-full" disabled={loading || !!currentRide}>
                    {loading ? "Booking..." : currentRide ? "Ride in Progress" : "Book Ride"}
                  </Button>
                </form>
              </CardContent>
            </Card>
                      ...
                      <div key={ride.id} className="border rounded-lg p-3">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="font-medium capitalize">{ride.rideType}</span>
                          </div>
                          <Badge className={statusColors[ride.status]}>{ride.status.replace("-", " ")}</Badge>
                        </div>
