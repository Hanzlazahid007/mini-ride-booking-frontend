...
import { useToast } from "@/hooks/use-toast"
import { rideAPI } from "@/lib/api"
import { Car, Bike, Truck, MapPin, User, Phone, Clock, RefreshCw } from "lucide-react"
import Navigation from "@/components/navigation"
import { useRouter } from "next/navigation"
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@/components/ui/select"
...
export default function RiderPage() {
  ...
  const fetchAvailableRides = useCallback(async (isAutoRefresh = false) => {
    ...
    try {
      ...
      if (isAutoRefresh && availableRides.length > 0) {
        ...
        const newRides = data.filter(ride => !currentRideIds.includes(ride.id))
        
        if (newRides.length > 0) {
          toast({
            title: "New Ride Request!",
            description: `${newRides.length} new ride request${newRides.length > 1 ? 's' : ''} available`,
            duration: 5000,
          })
        }
      }
                  ...
                  <Card>
                    <CardContent className="py-8">
                      ...
                      <p className="text-center text-sm text-gray-400 mt-2">
                        {autoRefresh ? "Auto-refreshing every 30 seconds..." : "Enable auto-refresh to check for new rides"}
                      </p>
                    </CardContent>
                  </Card>
                ) : (
